...
from skeletonization.connection_skeleton import connect_pcd, \
...
def get_skeleton_pcd(points, voxel_size, try_make_skeleton=False, plot_iter_pcd=False, iter=3, verbose=False):
    h0 = get_h0(points) / 5
    h = h0
    if verbose:
        print("::h0:", h0)
    ...
    for i in range(iters):
        ...
        center_pcd.points = open3d.utility.Vector3dVector(centers)
        if plot_iter_pcd:
        ...
        for j in range(50):
            ...
            error = my_Centers.contract(points, local_indices, h, density_weights)
...
if __name__ == "__main__":
    ...
    if calculate_pcd:
        ...
        final_centers = get_skeleton_pcd(points, voxel_size,  plot_iter_pcd=False, iter=3)
    ...
    labels, vector_orientation = get_point_label(xyz, indices, distance, number_NN=3, sigma_threshold=0.8,
                                                 visualize_label=True)
    clusters, clusters_connection = \
        connect_pcd_branch(xyz, labels, vector_orientation, threshold, indices, distance, number_NN=2,
                           plot_branch=False)
