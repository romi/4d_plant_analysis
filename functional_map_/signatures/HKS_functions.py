...
def mesh_HKS(eigenvalues, eigenvectors, num_T, landmarks=None, k=None):
    assert eigenvalues is not None, "Eigenvalues should be processed"

    if k is None:
        k = len(eigenvalues)
    else:
        assert len(eigenvalues >= k), f"At least ${k}$ eigenvalues should be computed, not {len(eigenvalues)}"

    return auto_HKS(eigenvalues[:k], eigenvectors[:,:k], num_T, landmarks=landmarks, scaled=True)
