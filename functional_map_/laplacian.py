...
def get_faces_area(vertices, faces):
...
def get_vertex_area(vertices, faces):
    N = vertices.shape[0]
    faces_areas = get_faces_area(vertices, faces)  # (m,)
    I = np.concatenate([faces[:,0], faces[:,1], faces[:,2]])
    J = np.zeros_like(I)
    V = np.concatenate([faces_areas, faces_areas, faces_areas])
...
def dia_area_mat(vertices, faces, faces_areas=None):
    """
    Compute the diagonal matrix of lumped vertex area for mesh functional_map_.
...
def cotangent_weights(vertices, faces):
    """
    Compute the cotengenant weights matrix for mesh functional_map_.
...
def get_mesh_laplacian_spectrum(k, vertlist, facelist, fem_area=False):
    L, M = robust_laplacian.mesh_laplacian(vertlist, facelist)
    # W = cotangent_weights(vertlist, facelist)
    if fem_area:
        A = fem_area_mat(vertlist, facelist)
    else:
        A = dia_area_mat(vertlist, facelist)

    # If k is 0, stop here
    eigenvalues, eigenvectors = laplacian_spectrum(L, M, spectrum_size=k)
    return eigenvalues, eigenvectors
